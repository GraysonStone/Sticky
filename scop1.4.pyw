...
class Stick():
    ...
    bgcolor=""      #Color of the background of the sticky note
    textcolor=""    #Color of the Text sticky note
    dataArr=[]      #Holds an array of each individual event to be placed on the sticky note
    undatedDataArr=[]  #Holds an Array of individual events that are undated
    xlocation=""    #The default window x location
    ylocation=""    #The default window y location
    transparency=1.0   #Transparency value of the sticky note
    noteMain=""     #The window value to allow settings functions to reset the note when settings are altered
    invert=0        #The value that determins event order i.e. most recent first or last
    colorVal=0      #Used in settings to determine which color value should be changed to avoid recreating all color buttons
    exit=False      #determins if the window was stopped from the x being pressed or for a routine date update
    ...
    def __getData(self):
        ...
        undatedTask=open("/Users/Gray/Desktop/StickCharm/Undate.txt","r")
        ...
        for line in undatedTask:
            self.undatedDataArr.append(line[0:len(line)-1])
    ...
    def __fixData(self):
        ...
        undatedTask=open("/Users/Gray/Desktop/StickCharm/Undate.txt","w")
        for i in range(0,len(self.dataArr)):
            task.write(self.dataArr[i].entry+'\n')
            date.write(self.dataArr[i].date+'\n')
        for i in range(0,len(self.undatedDataArr)):
            undatedTask.write(self.undatedDataArr[i]+'\n')
        undatedTask.close()
        task.close()
        date.close()
    ...
    def __window(self):
        ...
        note=self.noteMain
        extra=0
        if len(self.undatedDataArr)!=0:
            extra=22
        note.geometry('%dx%d+%d+%d' % (350,len(self.dataArr)*22+50+len(self.undatedDataArr)*22+extra, int(self.xlocation), int(self.ylocation)))
        ...
        top.pack()
        body=Canvas(note,width=350,height=len(self.dataArr)*22+15+len(self.undatedDataArr)*22+extra,bg=self.bgcolor,highlightthickness=0)
        body.pack()
        ...
        body.create_line(0,22*(len(self.dataArr)+1)+10,350,22*(len(self.dataArr)+1)+10,fill=self.textcolor)

        for i in range(len(self.dataArr),len(self.dataArr)+len(self.undatedDataArr)):
            body.create_text(1,22*(i+2),text=self.undatedDataArr[i-len(self.dataArr)],anchor='w',font=("Courier", 12),fill=self.textcolor)
            body.create_line(0,22*(i+2)+10,350,22*(i+2)+10,fill=self.textcolor)
        
            
        note.after(7200000, lambda: note.destroy())
        note.mainloop()
        if self.exit==False:
                Stick()
    ...
    def __addEntry(self,event):
        ...
        def addU():
            self.undatedDataArr.append(e.get())
            print(self.undatedDataArr)
            self.__fixData()
            add.destroy()
            self.noteMain.destroy()
            self.dataArr = []
        ...
        add=Tk()
        add.geometry('%dx%d+%d+%d' % (250,250,300,300))
        add.title("Add Task")
        task=""
        e=Entry(add)
        e.place(x=30,y=10)
        undatedEntry=Button(add,text="Undated",command=addU)
        undatedEntry.place(x=170,y=220)
    ...
    def __delEntry(self,event):
        ...
        delList=[]
        delE=Tk()
        delE.geometry('%dx%d+%d+%d' % (250, 30*(len(self.dataArr)+len(self.undatedDataArr))+30, 300, 300))
        delE.title("Remove Task")
        ...
        for c in range(0,len(self.undatedDataArr)):
            l = Label(delE, text=self.undatedDataArr[c])
            l.bind("<Enter>", lambda event, c=c: on_enter(event, c+len(self.dataArr)))
            l.bind("<Leave>", lambda event, c=c: on_leave(event, c+len(self.dataArr)))
            l.bind("<Button-1>", lambda event, c=c: deleteU(event, c))
            delList.append(l)
            l.place(x=10, y=(c+len(self.dataArr)) * 30 + 30)
    ...
    def __setEntry(self,event):
        ...
        def color_bar():
            def get():
                ...
                if self.colorVal==0:
                    self.bgcolor='#%02x%02x%02x' % (w.get(), w2.get(), w3.get())
                elif self.colorVal==1:
                    self.topcolor='#%02x%02x%02x' % (w.get(), w2.get(), w3.get())
                elif self.colorVal==2:
                    self.textcolor='#%02x%02x%02x' % (w.get(), w2.get(), w3.get())
                self.__fixColors()
                self.noteMain.destroy()
                sett.destroy()
                colorB.destroy()
            ...
            colorB=Tk()
            colorB.geometry('%dx%d+%d+%d' % (240, 180, 300, 300))
            colorB.title("ColorBar")
            w=Scale(colorB,from_=1,to=255,orient=HORIZONTAL,length=230)
            w.set(1)
            w.place(x=5,y=5)
            w2=Scale(colorB,from_=1,to=255,orient=HORIZONTAL,length=230)
            w2.set(1)
            w2.place(x=5,y=50)
            w3=Scale(colorB,from_=1,to=255,orient=HORIZONTAL,length=230)
            w3.set(1)
            w3.place(x=5,y=95)
            App=Button(colorB,text="Apply",command=get)
            App.place(x=90,y=140)
            colorB.mainloop()
        ...
        def startloc_sett():
            ...
            place.title("Default Window Location")
        ...
        sett=Tk()
        sett.geometry('%dx%d+%d+%d' % (300, 417, 300, 300))
        sett.title("Settings")
        ...
        bar=Button(sett,text="RGB Custom",command=color_bar)
        bar.place(x=20,y=382)
